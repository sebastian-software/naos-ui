...
| [Lit](./lit) | Web Components library | Lit is a runtime library (no build step) with tagged templates; Naos is a compiler (no template runtime) with JSX + signals |
...
| [React](./react) | App runtime library | A different corner: React is a VDOM app runtime; Naos is complementary — build components in Naos, consume them in React |
