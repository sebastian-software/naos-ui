...
```js
import { K, createKernel, connect, disconnect, attrChanged, defineProps,
         defineComponent, lazySheet } from "@naos-ui/runtime/internal"

const SPEC = {
  defaults: { disabled: false, label: "Toggle" },
  props: { /* coerce/reflect per prop */ },
  attrs: { /* parse per observed attribute */ },
  styles: lazySheet([css]),
  initState(k) { k.state.pressed = false },
  mount(k) { /* component-specific DOM construction */ },
  update(k, dirty) { /* component-specific guarded updates */ },
  effects(k, dirty) { /* component-specific effect bodies */ },
}

class ToggleElement extends HTMLElement {
  static observedAttributes = ["disabled", "label"]
  constructor() { super(); this[K] = createKernel(this, SPEC) }
  connectedCallback() { connect(this[K]) }
  disconnectedCallback() { disconnect(this[K]) }
  attributeChangedCallback(n, o, v) { attrChanged(this[K], n, o, v) }
}
